...
getUserProfileInfo(UserProfileProvider providerValue) async {
...
getStoreProfileInfo(StoreProfileProvider providerValue) async {
  String userId = FirebaseAuth.instance.currentUser!.uid;
  var fireAuth =
      FirebaseFirestore.instance.collection("store").doc(userId).get();
  fireAuth.then(
    (value1) => value1.data()?.forEach(
      (key, value) {
        if (key == "name") {
          ...
          providerValue.getLicense(value);
        } else if (key == "pic") {
          providerValue.getPic(value);
        }
      },
